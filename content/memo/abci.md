...
title: 'スパコン利用メモ（ABCI）'
date: '2026-01-12'
description: 'ABCI環境における，環境構築からジョブ投入までの基本ワークフローまとめ．'
---


これは以前，[ABCI](https://docs.abci.ai/v3/ja/)と呼ばれるAI学習等に使用されるスパコンを触る機会がありましたので，その際に触れたプラクティスです．
共有サーバ独特の作法が必要となるため，環境構築からジョブ実行までの手順を整理します．

## 1. 基本思想：ログインノードと計算ノード

スパコンは「ログインノード」と「計算ノード」で役割が完全に分かれています．
この区別を理解することが最初のステップです．

| ノード | 役割 | 禁止事項 / 特徴 |
| :--- | :--- | :--- |
| **ログインノード** | SSHで最初に接続する場所。<br>ファイル編集、コンパイル、`pip install`、ジョブ投入などに使用． | **重い計算（学習など）は禁止．**<br>アカウント停止の対象になる場合があるため注意． |
| **計算ノード** | 実際のAI学習が行われる場所．<br>A100 GPUなどが搭載されている． | ジョブ管理システム（`qsub`）経由でのみ利用可能．<br>**ログインノードの環境は引き継がれない**（都度ロードが必要）． |

## 2. 環境構築：`module`と`venv`

ABCIでは、システム側が提供する **`module`** と，ユーザー空間の **`venv`** を組み合わせて環境を作ります．
`conda` はシステム環境と競合しやすいため，**非推奨**とされることが多いです．

### 2-1. module コマンドの基本

システムにインストールされたコンパイラやライブラリ（Python, CUDAなど）を管理するコマンドです．

* **`module avail`**: 利用可能なモジュール一覧を表示．
* **`module load [名前]`**: 指定したモジュールをロードする．
* **`module list`**: 現在ロードされているモジュールを確認する．
* **`module purge`**: ロード中のモジュールを全解除する（環境リセット用）．

### 2-2. 必要なモジュール

深層学習を行う場合，主に以下のモジュール群（`/gpgpu` や `/devtools` カテゴリ）を使用します．

* **`python/...`**: Python本体．
* **`cuda/...`**: GPUドライバ周り．PyTorchのバージョンと合わせる必要がある．
* **`cudnn/...`**: Deep Learning高速化ライブラリ．
* **`nccl/...`**: 複数GPU間の通信ライブラリ（分散学習に必須）．
...
**重要:** GPUを使わないテスト（`import`確認など）は，課金レートが安いCPUノード（`rt_HC`など）をリクエストしてください．
